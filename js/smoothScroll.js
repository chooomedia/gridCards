(function (root, smoothScroll) {
  ...
  let smoothScroll = function(el, duration, callback, context){
  ...
  let linkHandler = function(ev) {
      if (!ev.defaultPrevented) {
          ...
          let node = document.getElementById(this.hash.substring(1))
          if (!node) return; // Do not scroll to non-existing node
  
          smoothScroll(node, 800, function (el) {
  ...
  document.addEventListener("DOMContentLoaded", function () {
      let internal = document.querySelectorAll('a[href^="#"]:not([href="#"])'), a;
